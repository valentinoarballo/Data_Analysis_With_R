...
cat("Descarga el archivo CSV de Google Drive")
url <- "https://drive.google.com/u/0/uc?id=1VZbVd99DpRkph2HXODkeCer1TZlnr887&export=download"
file_path <- tempfile(fileext = ".csv")
download.file(url, file_path)

cat("Lee el archivo CSV en R")
DB <- read.csv(file_path, sep = ";")
...
ggplot(data=predicciondfeuro_rbind, aes(x=fechasdf, y=preciosdf)) +
  geom_area(stat = "identity", fill = 4, alpha = 0.5, color = 1, lwd = 0.5, linetype = 1) +
